import {atom, selector, atomFamily, selectorFamily, noWait} from 'recoil';
import {
    SYMBOLHIGHLIGHTS_RECOIL,
    RULEDOTHIGHLIGHTS_RECOIL,
    RULEBACKGROUNDHIGHLIGHTS_RECOIL,
} from '../types/propTypes';
import {colorPaletteState} from './settingsState';
...
export const setReasonHighlightsCallback =
    ...
    async (transformationHash, symbolUuid, data) => {
        ...
        set(ruleDotHighlightsStateFamily(transformationHash), newRuleDotHighlights);
...
export const clearAllHighlightsCallback = ({snapshot, set}) => async () => {
    // Search
    set(
        symbolSearchHighlightsState,
        []
...
export const handleSearchResultSuggestionsCallback =
    ...
    async () => {
        async function setSymbolSearchHighlights(newValue) {
            ...
            const selectedBranch = await snapshot.getPromise(selectedBranchState);
            const colorPalette = await snapshot.getPromise(colorPaletteState);
            const nextColor = getNextColor(
        ...
        const selectedSuggestion = await snapshot.getPromise(selectedSuggestionState);
